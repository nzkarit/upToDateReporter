...
*/
    ...
    $smarty = new Smarty_Header('view_application_csv.tpl');
    ...
    $query = 'SELECT application_name FROM application WHERE application_id = ? AND (user_id = ? OR application_sample = 1);';
    $statement = $connection->prepare($query);
    $statement->bind_param('ii', $application_id, $user_id);
    $application_id = $id;
    $user_id = $user_id;
    $statement->execute();
    ...
    $smarty->assign('application_id', $application_id);
    ...
    $query = <<<'EOT'
        SELECT al.applib_id, al.applib_description, l.library_name, l.library_eol, library_eol_date, v.version_name, v.version_release_date, v2.version_name, v2.version_release_date, v3.version_name, v3.version_release_date
	        ...
	        WHERE al.application_id = ? AND (a.user_id = ? OR a.application_sample =1)
...
EOT;
    ...
    $filename = $application_name.'-'.date('Y-m-d-His').'.csv';
